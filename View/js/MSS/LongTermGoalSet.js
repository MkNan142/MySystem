...
$(function () {
  ...
  $('#ltg_start_time,#ltg_end_time').daterangepicker({
...
function saveform() {
  ...
  var doMissionAction = '';
  var FinishAlert = '';
  if ($('#ltg_action').val() == 'INS') {
    doMissionAction = 'insLongTermMission';
    FinishAlert = '新增完成';
  } else if ($('#ltg_action').val() == 'UPD') {
    doMissionAction = 'updLongTermMission';
    FinishAlert = '修改完成';
  }
  //return false;
...
function getRecord(pageNum) {
  ...
  sch_val['ltg_status'] = $('#schMissionStatus').val();
  $.each($('.ms_long_table'), function (k, v) {
    if (!$(this).hasClass('sorting')) {
      if ($(this).hasClass('sorting_asc')) {
        sch_val['orderby'] = $(this).attr('id').replace("ms_long_table_", "");
    ...
    success: function (data) {
      ...
      $.each(data['row'], function (k, v) {
        if (count_data % 2 == 0) {
          ...
          ms_long_table_body_html += '<tr role="row" class="even">';
        }
        var show_data = '';
        $.each(table_field, function (k1, v1) {
          if (v1 == 'edit') {
            show_data = '<div class="btn-group">';
            show_data += '<button type="button" class="btn btn-default" onclick="editMission(' + v['ltg_id'] + ')"><i class="far fa-edit"></i></button>';
            show_data += '<button type="button" class="btn btn-default" onclick="delMission(' + v['ltg_id'] + ')"><i class="far fa-trash-alt"></i></button>';
            show_data += '</div>';
          } else if (v1 == 'ltg_status') {
            switch (v[v1]) {
              case '0':
                show_data = '準備中';
                break;
          ...
          ms_long_table_body_html += '<td>' + show_data + '</td>';
        })

        count_data++;
      })
      //console.log(ms_long_table_body_html);
      //console.log(data['rowcount']);
      $('#ms_long_table_body').html(ms_long_table_body_html);
      ...
      $('#ms_long_table_info').html('Showing ' + parseInt((pageNum - 1) * 10 + 1) + ' to ' + pageNum * 10 + ' of ' + data['rowcount'] + ' entries');
...
function editMission(ltg_id) {
  ...
  $.ajax({
    ...
    url: url,
    dataType: "json",
    data: { data: sch_val, doMissionAction: 'getLongTermMissionByID' }, // serializes the form's elements.
    success: function (data) {
...
function delMission(ltg_id) {
  ...
  $.ajax({
    ...
    url: url,
    dataType: "json",
    data: { data: sch_val, doMissionAction: 'delLongTermMissionByID' }, // serializes the form's elements.
    success: function (data) {
      alert('刪除完成');
      //console.log(data);
      getRecord(1);
...
$('.ms_long_table').not('.no_sort').on('click', function () {
  //console.log($(this).attr('id'));
  if ($(this).hasClass('sorting') || $(this).hasClass('sorting_asc')) {
    $('.ms_long_table').not('.no_sort').removeClass('sorting sorting_asc sorting_desc');
    $('.ms_long_table').not('.no_sort').addClass('sorting');
